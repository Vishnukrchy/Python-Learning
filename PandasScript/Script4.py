...
# Similar trades (matching any word from newTrade with dbTrade)
similar_trades = []
for new_trade in new_trade_list:
    ...
    for db_trade in db_trade_list:
        db_trade_words = db_trade.lower().split()  # Split the dbTrade into individual words
        # Check if any word from new_trade matches any word in db_trade
        if any(word in db_trade_words for word in new_trade_words):
...
output_file_path = 'PandasScript/comparison6_results.csv'  # Specify your desired output file name
